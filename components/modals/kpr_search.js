...
module.exports = {
    async execute(interaction){
        ...
        let pg = parseInt(interaction.fields.getTextInputValue('kpr_search_pageInput')) - 1;

        if(pg > Math.floor(stats.length / 10)) pg = Math.floor(stats.length / 10);
        if(pg < 0) pg = 0;

        if(isNaN(pg)){
            const username = interaction.fields.getTextInputValue('kpr_search_usernameInput').toLowerCase();
