...
async function listWins(page, stats, interaction){
    ...
    for(let i in set){
        ...
        let wins = set[i].weekWins;

        embedFields.push({
            name: `${pos}: ${set[i].username} - \`${wins}\``, 
            value: `Participated: \`${set[i].participated}\`, Game Wins: \`${set[i].gameWins}\``
        });
    }

    const embed = [
        new EmbedBuilder()
            .setTitle(`Week Wins Leaderboard: Page ${page + 1}`)
            .setColor('#FEF853')
    ...
    if(interaction.message == undefined){
        ...
        setTimeout(async () => {
            const offButtons = [
                new ButtonBuilder()
                    .setEmoji('⏮️')
                    .setCustomId('wins_first_disabled')
                    .setStyle(ButtonStyle.Primary)
                    .setDisabled(true),
        
                new ButtonBuilder()
                    .setEmoji('◀️')
                    .setCustomId('wins_back_disabled')
                    .setStyle(ButtonStyle.Primary)
                    .setDisabled(true),
        
                new ButtonBuilder()
                    .setEmoji('#️⃣')
                    .setCustomId('wins_search_disabled')
                    .setStyle(ButtonStyle.Primary)
                    .setDisabled(true),
        
                new ButtonBuilder()
                    .setEmoji('▶️')
                    .setCustomId('wins_next_disabled')
                    .setStyle(ButtonStyle.Primary)
                    .setDisabled(true),
        
                new ButtonBuilder()
                    .setEmoji('⏭️')
                    .setCustomId('wins_last_disabled')
                    .setStyle(ButtonStyle.Primary)
                    .setDisabled(true),
            ];
